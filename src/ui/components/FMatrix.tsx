...
export function FMatrix({ game, txs }: FMatrixProps) {
  ...
  return (
    <div className="max-h-[70svh] overflow-auto rounded-2xl border border-mist-800 bg-mist-950/80 shadow-lg shadow-mist-950/40">
      <table className="f-matrix min-w-full text-left text-sm">
        <thead>
          <tr>
            <th className="corner border-b border-r border-mist-800 bg-mist-950 px-2 py-2 text-[10px] uppercase tracking-wide text-mist-400">
              Active \ Passive
            </th>
            {game.players.map((player, index) => (
              <th
                key={player.id}
                className="border-b border-mist-800 bg-mist-950 px-2 py-2 align-bottom"
              >
                <div className="text-xs font-semibold text-mist-100">
                  {getPlayerName(game, player.id)}
                </div>
                <div className="text-[10px] text-mist-400">Seat {index + 1}</div>
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {game.players.map((rowPlayer, rowIndex) => (
            <tr key={rowPlayer.id}>
              <th className="border-r border-mist-800 bg-mist-950 px-2 py-2">
                <div className="text-xs font-semibold text-mist-100">
                  {getPlayerName(game, rowPlayer.id)}
                </div>
                <div className="text-[10px] text-mist-400">Seat {rowIndex + 1}</div>
              </th>
              {game.players.map((columnPlayer, columnIndex) => {
                ...
                return (
                  <td
                    key={columnPlayer.id}
                    className="border-b border-r border-mist-800 px-2 py-2 align-top font-mono text-[11px] leading-tight text-mist-100"
                    style={{
