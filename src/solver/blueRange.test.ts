...
describe('getBlueRangeStepDelta', () => {
  it('follows the standard BotC good-player step pattern', () => {
    ...
    expect(getBlueRangeStepDelta(8, 9)).toBe(0)
    expect(getBlueRangeStepDelta(9, 10)).toBe(2)
    expect(getBlueRangeStepDelta(10, 11)).toBe(0)
    expect(getBlueRangeStepDelta(11, 12)).toBe(0)
    expect(getBlueRangeStepDelta(12, 13)).toBe(2)
...
describe('shiftBlueRangeWithPlayerCount', () => {
  it('shifts both boundaries by the mapped delta', () => {
    ...
    expect(shiftBlueRangeWithPlayerCount(2, 5, 9, 10)).toEqual({ min: 4, max: 7 })
    expect(shiftBlueRangeWithPlayerCount(2, 5, 10, 9)).toEqual({ min: 0, max: 3 })
  })

  it('clamps shifted boundaries to the new player count', () => {
    expect(shiftBlueRangeWithPlayerCount(0, 5, 5, 6)).toEqual({ min: 1, max: 6 })
    expect(shiftBlueRangeWithPlayerCount(8, 9, 13, 12)).toEqual({ min: 6, max: 7 })
  })
})
